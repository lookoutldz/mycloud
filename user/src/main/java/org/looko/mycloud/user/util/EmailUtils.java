...
import org.looko.mycloud.user.exception.EmailException;
...
public class EmailUtils {
    ...
    public void postValidcodeEmail(BusinessTypeEnum businessTypeEnum, String to, String validcode) {
        //发到 MQ, 解耦
        try {
     ...
     */
    private SimpleMailMessage genValidcodeEmail(BusinessTypeEnum businessTypeEnum, String to, String validcode) {
        String subject;
        String content;
        switch (businessTypeEnum) {
            case VALIDCODE_LOGIN -> {
                subject = "登录 MyCloud";
                content = "您正在进行登录 MyCloud 的操作, 验证码为 " + validcode + " , " + Constants.validcodeExpireMinutes
                        + " 分钟内有效。\n若非您的操作请忽略此封邮件。\n\tMyCloud 团队";
            }
            case VALIDCODE_REGISTER -> {
                subject = "注册 MyCloud";
                content = "您正在进行注册 MyCloud 账户的操作, 验证码为 " + validcode + " , " + Constants.validcodeExpireMinutes
                        + " 分钟内有效。\n若非您的操作请忽略此封邮件。\n\tMyCloud 团队";
            }
            case VALIDCODE_RESET_PASSWORD -> {
                subject = "重置 MyCloud 密码";
                content = "您正在进行 MyCloud 账户的重置密码操作, 验证码为 " + validcode + " , " + Constants.validcodeExpireMinutes
                        + " 分钟内有效。\n若非您的操作请忽略此封邮件。\n\tMyCloud 团队";
            }
            default -> throw new EmailException(businessTypeEnum.name() + "类型业务尚无邮件模板。");
        }
